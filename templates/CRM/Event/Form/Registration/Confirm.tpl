...
<div class="crm-block crm-event-confirm-form-block">
                ...
                {if $participant.additionalCustomPre}
                    {foreach from=$participant.additionalCustomPre item=value key=field}
							...
							<div class="crm-section {$field}-section">
                                <div class="label"><label for="{$field}">{$field}</label></div>
    ...
    <div class="messages status section continue_message-section">
        <p>
        {ts}Your registration will not be submitted until you click the <strong>Continue</strong> button. Please click the button one time only. Please note our cancellation policy below before booking.{/ts}
        </p>
    </div>
    {/if}    
