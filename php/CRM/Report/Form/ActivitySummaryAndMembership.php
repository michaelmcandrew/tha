...
class CRM_Report_Form_ActivitySummaryAndMembership extends CRM_Report_Form {
    ...
    function from( ) {
        ...
        FROM civicrm_activity {$this->_aliases['civicrm_activity']}
             ...
             LEFT JOIN civicrm_membership 
                    ON civicrm_membership.contact_id = {$this->_aliases['civicrm_activity']}.source_contact_id         
                    ";
    ...
    function alterDisplay( &$rows ) {
        ...
        foreach ( $rows as $rowNum => $row ) {
            ...
            if ( array_key_exists('civicrm_membership_membership_type', $row ) ) {
                if ( $value = $row['civicrm_membership_membership_type'] ) {
                    $rows[$rowNum]['civicrm_membership_membership_type'] = $membershipTypes[$value];
                    $entryFound = true;
                }
            }
            
            if ( array_key_exists('civicrm_membership_membership_status', $row ) ) {
                if ( $value = $row['civicrm_membership_membership_status'] ) {
                    $rows[$rowNum]['civicrm_membership_membership_status'] = $membershipStatuses[$value];
                    $entryFound = true;
                }
            }
